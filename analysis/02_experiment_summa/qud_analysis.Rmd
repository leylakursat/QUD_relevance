...
**Non-unique participants (remove both attempts):**
...
```{r slowtrial_exclude, echo=FALSE}
...
workers <- df[!duplicated(df$workerid), ]
table(workers$Answer.condition)
```
...
```{r response_type_age, echo=FALSE, fig.height=3, fig.width=9}
rtype = critical %>%
  mutate(semantic = ifelse(key=="Yes",1,0)) %>%
...
```{r response_time_age_responder, echo=FALSE, fig.height=5, fig.width=8}
agr2 = critical %>%
  merge(responder[ ,c("workerid","responder_type")], by="workerid",all.x=TRUE) %>%
  merge(demo[ ,c("workerid","age")], by="workerid",all.x=TRUE) %>%
  mutate(age_bucket = ifelse(age<=25,"0-25",ifelse(age<44,"26-44", ifelse(age>44,"45+",NA))))%>%
