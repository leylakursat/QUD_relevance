...
#ggsave(responder2, file="../graphs/responder.pdf",width=8,height=4)
```
...
View(critical) # has age and age_group

#1) Mixed effects logistic regression predicting response from fixed effects of QUD, age and their interaction
m = glmer(key ~ Answer.condition*age + (1|workerid), data=critical, family="binomial")
summary(m)

#2) Linear mixed effects regression predicting log response time from fixed effects of QUD, response type, age and their interaction
m.all=lmer(logRT ~ Answer.condition*key*age + (1|workerid), data=critical)
summary(m.all)
