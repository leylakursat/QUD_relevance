...
# exclude no-QUD
df = df %>%
  filter(str_detect(Answer.condition,"no_QUD", negate = TRUE))
demo = demo %>%
  filter(str_detect(speaker_cond,"noQUD", negate = TRUE))
```
### Before Exclusions
...
yes_list = c("summa2","summa5","summa8","summa11",
             "all13",
             "none0",
             "two2","five5","eight8","eleven11")

# if we don't exlude any stimuli from accuracy analysis -- 268 people excluded
#seven8 and up --262 people excluded
exclude_list_1 = c("seven11","eight8","eight13","ten13","eleven8","eleven11","thirteen8")
#ambiguous stimuli -- 260 people excluded
exclude_list_2 = c("eight13","four5","seven11","two5","one11","ten13","five13","two8")
# if both lists are excluded -- 255 people excluded
...
ggplot(non_critical,aes(x=image, y=count ,fill=key))+
  ...
  ylab("count") +
  labs(fill="response") +
  facet_wrap(~audio, scales="free_y")
...
no_list = c("summa0","all0","all2","all5","all8","all11","none2","none8","none13",
            "three0","five0","nine5","ten5","eleven0","eleven8","twelve2","thirteen8")

amb_list = c("two5","two8","one11","four5","seven11","five13","eight13","ten13")

non_critical_2 = trials %>%
  mutate(concat = str_c(str_sub(audio,1,end=-5),image)) %>%
  filter(str_detect(concat,"summa13", negate = TRUE)) %>%
  filter(key != "Late") %>%
  mutate(expectedAns = ifelse(concat %in% yes_list, "yes", ifelse(concat %in% no_list, "no", ifelse(concat %in% amb_list, "amb", "NA")))) %>%
  group_by(expectedAns,key,Answer.condition) %>%
  summarize(count = n()) %>%
  group_by(expectedAns, Answer.condition) %>%
  mutate(sum = sum(count), proportion = count/sum)

expected_resp = ggplot(non_critical_2,aes(x=key, y=proportion, fill=as.factor(expectedAns)))+
  geom_bar(stat = "identity", position=position_dodge(.9)) +
  ylab("proportion") +
...
#ggsave(expected_resp, file="../graphs/expected_resp.pdf",width=4,height=2)
...
expected_resp_rt = ggplot(non_critical_3,aes(x=key, y=Mean, fill=as.factor(expectedAns)))+
  geom_bar(stat = "identity", position=position_dodge(.9)) +
  ylab("mean rt") +
  geom_text(aes(label=count, y = 5),position=position_dodge(width=1),vjust=0,size=3) +
  facet_wrap(~Answer.condition)
...
nrow(critical) #6903 #6929

#exclude no-QUD
critical = critical %>%
  filter(str_detect(Answer.condition,"no_QUD", negate = TRUE))

#write.csv(critical, file = "experiment4_critical.csv")
```
...
pragmatic = ggplot(rtype2, aes(x = Answer.condition, y=Proportion)) +
  ...
  xlab("") +
  theme(axis.text.y=element_text(size=12), axis.title=element_text(size=10), axis.text.x=element_text(size=8))
...
d = critical %>%
  ...
  summarise(numPragmatic = n()) %>%
  filter(key=="No") %>%
  merge(critical[ ,c("workerid","Answer.condition")], by="workerid",all.x=TRUE) %>%
  unique() %>%
  group_by(Answer.condition,numPragmatic) %>%
  count() %>%
  group_by(Answer.condition) %>%
  mutate(total_condition = sum(n)) %>%
  mutate(prop_condition = n/total_condition) %>%
  group_by(numPragmatic) %>%
  mutate(total_numPragmatic = sum(n)) %>%
  mutate(prop_numPragmatic = n/total_numPragmatic)

proportion = ggplot(d, aes(x=numPragmatic, y=prop_numPragmatic, fill=Answer.condition)) +
  ...
  theme(axis.text.y=element_text(size=10), axis.title=element_text(size=7), axis.text.x=element_text(size=10))

proportion

#ggsave(proportion, file="../graphs/pragmatic_proportion_qud.pdf",width=4,height=2)

proportion2 = ggplot(d, aes(x=numPragmatic, y=prop_condition, fill=Answer.condition)) +
  geom_bar(stat="identity", position = position_dodge(.6), width = 0.6) +
  scale_fill_manual(values = cond_col) +
  xlab("Number of pragmatic responses") +
  ylab("Proportion of participants in QUD") +
  labs(fill="QUD") +
  scale_x_continuous(breaks=c(0:8)) +
  #scale_y_continuous(limits=c(0:1)) +
  theme(axis.text.y=element_text(size=10), axis.title=element_text(size=7), axis.text.x=element_text(size=10))+
...
#ggsave(proportion2, file="../graphs/pragmatic_proportion_answer.pdf",width=4,height=2)
...
#ggsave(response_time, file="../graphs/response_time.pdf",width=4,height=2)
```
...
**EXTRA: Pragmaticity and response time**
```{r semanticity_rtime, eval=FALSE, fig.height=3, fig.width=6, include=FALSE}
worker_prag = critical %>%
  group_by(workerid,key, .drop =FALSE) %>%
  summarize(numPragmatic = n()) %>%
  filter(key=="No")

pragmaticity = critical %>%
  merge(worker_prag[ ,c("workerid","numPragmatic")], by="workerid",all.x=TRUE) %>%
  #filter(key=="No") %>%
  group_by(numPragmatic,Answer.condition) %>%
  summarize(Mean = mean(rt))

prg2 = ggplot(pragmaticity, aes(x=numPragmatic, y=Mean)) +
  geom_bar(stat = "identity") +
  scale_color_manual(key_col) +
  xlab("Number of pragmatic responses") +
  ylab("mean RT of all responses") +
  #ylab("mean RT of responses") +
  scale_x_continuous(breaks=c(0:8)) +
  facet_wrap(~Answer.condition)
  
prg2

ggsave(prg2, file="../graphs/pragmaticity_rt_qud.pdf",width=4,height=2)
...
contrasts(critical_cresponder$key)
contrasts(critical_cresponder$Answer.condition)
contrasts(critical_cresponder$responder_type)

table(critical_cresponder$responder_type)
table(critical_cresponder$key)

m4 = lmer(logRT ~ Answer.condition*cresponder_type*ckey + (1|workerid), data=critical_cresponder)
summary(m4)

#
m4 = lmer(logRT ~ Answer.condition*cresponder_type*ckey + (1+ckey|workerid), data=critical_cresponder)
summary(m4)

m5 = lmer(logRT ~ cresponder_type*ckey + (1+ckey|workerid), data=critical_cresponder)
summary(m5)

anova(m4,m6)

m6 = lmer(logRT ~ Answer.condition*ckey+Answer.condition*cresponder_type + Answer.condition+ ckey:cresponder_type + (1+ckey|workerid), data=critical_cresponder)
summary(m6)
...
#semanticity instead of responder type 
critical_sem = critical %>%
  merge(responder[ ,c("workerid","n")], by="workerid",all.x=TRUE) %>%
  mutate(ckey = as.numeric(key) - mean(as.numeric(key)))
  
View(responder)

m6 = lmer(logRT ~ Answer.condition*n*ckey + (1|workerid), data=critical_sem)
summary(m6)
```
