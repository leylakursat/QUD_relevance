...
#exc = demo[!complete.cases(demo$age),] #remove NA

#demo2 = demo[!(demo$workerid %in% exc$workerid),]
...
ggplot(agr, aes(fill=key,x=Answer.condition,y=Mean)) +
  geom_bar(stat="identity",position=position_dodge(.9)) +
  scale_fill_manual(values = key_col) +
  geom_errorbar(aes(ymin = YMin, ymax = YMax),width=.25,position=position_dodge(.9)) +
  ylab("mean rawRT")+
...
ggplot(agr3, aes(fill=key,x=Answer.condition,y=Mean)) +
  geom_bar(stat="identity",position=position_dodge(.9)) +
  scale_fill_manual(values = key_col) +
  geom_errorbar(aes(ymin = YMin, ymax = YMax),width=.25,position=position_dodge(.9)) +
