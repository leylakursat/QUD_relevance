---
title: 'Experiment 6: summa any-QUD (merge with all)'
...
df_all = read.csv("04_results_formatted.csv", header = TRUE)
demo_all = read.csv("04_subject_info.csv", header = TRUE)

df_any = read.csv("06_results_formatted.csv", header = TRUE)
demo_any = read.csv("06_subject_info.csv", header = TRUE)

df_all = df_all %>%
  filter(str_detect(Answer.condition,"any_QUD", negate = TRUE)) %>%
  filter(str_detect(Answer.condition,"no_QUD", negate = TRUE)) %>%
  mutate(workerid = workerid + 400) %>%
  select(-c(speaker_cond))

demo_all = demo_all %>%
  filter(str_detect(speaker_cond,"anyQUD", negate = TRUE)) %>%
  filter(str_detect(speaker_cond,"noQUD", negate = TRUE)) %>%
  mutate(workerid = workerid + 400) %>%
  select(-c(speaker_cond))

df = rbind(df_all,df_any)
demo = rbind(demo_all,demo_any)
...
#latest exp
deneme1 = df %>%
  select(workerid,Answer.condition) %>%
  unique()
...
#write.csv(critical, file = "experiment6_critical.csv")
...
#nadiffs  = diffs[is.na(diffs$Diff),]
#nadiffpeople = unique(nadiffs$workerid)

#nadiff = critical %>%
#  filter(workerid %in%  nadiffpeople)  %>%
#  droplevels() %>%
#  group_by(workerid,key)  %>%
#  summarize(Mean = mean(rt))
...
#ggplot(nadiff, aes(x=Mean,fill=key)) +
...
**EXTRA: Semanticity and response time**
```{r semanticity_rtime, eval=FALSE, fig.height=3, fig.width=6, include=FALSE}
sem = critical %>%
  group_by(workerid) %>%
  summarize(Mean=mean(rt)) %>%
  merge(d[ ,c("workerid","n")], by="workerid",all.x=TRUE) %>%
  group_by(n) %>%
  summarize(Mean=mean(Mean))

ggplot(sem, aes(x=n, y=Mean)) +
  geom_bar(stat = "identity") +
...
prr = ggplot(pragmaticity, aes(x=numPragmatic, y=Mean)) +
  geom_bar(stat = "identity") +
  scale_color_manual(key_col) +
  xlab("Number of pragmatic responses") +
...
ggsave(prr, file="../graphs/pragmaticity_rt_qud.pdf",width=4,height=2)
...
#m3=lmer(logRT ~ key + (1|workerid), data=critical_ckey)
#summary(m3)
...
#4.SEMANTICITY - Mixed effects linear regression model with random by-participant intercepts predicting log-transformed response time from fixed effects of QUD, response type, semanticity and their interaction
